...
## 2024-05-24 - Information Disclosure via Insecure Direct Object Reference (IDOR) on Category Listings
**Vulnerability:** Future/embargoed articles (with a `lockedUntil` date in the future) were successfully hiding their markdown content via a `LockedArticleView` check inside the individual article page. However, the metadata (title, description, and existence) of these embargoed articles was still being leaked on the parent Category Listing page (`src/app/[locale]/wiki/[category]/page.tsx`), as the list rendering logic did not filter out future-locked items.
**Learning:** Security checks implemented at the individual resource view level are insufficient if the collection listing endpoint/view does not enforce the same visibility constraints. This leads to partial information disclosure.
**Prevention:** Implement collection-level filtering to ensure that items inaccessible to the user are completely omitted from lists, searches, and related content sections.
